...
-- 2. 직원번호, 이름 , 급여, 각 사원 급여에 100을 더하시오
...
from employees;     -- 각 행에 있는 열에 수식을 사용가능

-- 부서정보
select * from departments;

-- 4. 주소, 부서번호, 부서장 번호, 부서명순으로 출력
...
-- 6. 연산 : 
-- 사원테이블에서 사원번호, 부서번호, 직무, 연봉을 출력하는데 매월 300씩 보너스가 지급이 된다.
select employee_id, department_id, job_id, salary, (salary + 300) * 12
from employees;

-- 7. 사원테이블에서 사원번호, 부서번호, 직무, 연봉을 출력하는데 마지막 달에 300이 특별 보너스로 지급이 된다.
select employee_id, department_id, job_id, salary, salary * 12 + 300
from employees;

-- 8. 사원테이블에서 사원번호, 성, 이메일, 급여에 대한 커미션이 30%씩 지급될 때 커미션까지 출력하시오.
...
-- 10. 사원테이블에서 직원번호, 이메일, 이름, 입사일, 급여, 커미션퍼센트를 출력하는데 각 사원이 받는 커미션의 금액도 같이 출력하시오.
select employee_id, email, first_name, hire_date, salary, commission_pct, salary * commission_pct
from employees;

select employee_id, email, hire_date, salary + null     -- salary는 전부 null이 된다.
from employees;

select employee_id, email, hire_date, department_id, job_id, commission_pct * null      -- null에는 어떠한 값을 +, * 전부 null이 된다.
from employees;

-- 11. 직원번호, 이메일, 이름, 입사일, 연락처, 부서번호, 급여, 커미션퍼센트, 그리고 커미션이 포함된 연봉을 출력하시오.
select employee_id, email, last_name, hire_date, phone_number, department_id, salary, commission_pct,  (salary * ( 1 + commission_pct) * 12)
-- (salary + salary * commission_pct) * 12
from employees;

-- 12. heading name에 별칭 주기
select employee_id eid, email, last_name lname, hire_date hd, phone_number ph, department_id did, salary sal, commission_pct comm, (salary + salary * commission_pct) * 12 year_sal
from employees;
...
select employee_id, first_name, salary, commission_pct, salary + salary * 12 as "year sal", salary + salary * 12 "year sal", salary + salary * 12 as "Year Sal"
from employees;
--- 자바에서 문제가 생길 수 있어서 별칭에 공백문자를 쓰지 않고 대소문자를 구별하지 않는 것이 좋다.

select '홍길동' from dual;     -- 데이터베이스에 있는 모든 데이터는 ''로 처리한다. "" 사용하지 않는다.
...
from employees;
-- 행의 값이 모두 같은 경우 제거가 되지만 적어도 하나의 컬럼 값이 다르다면 다르게 출력된다.

--      107                 12
-- select job_id, DISTINCT department_id;
-- DISTINCT는 중간에 사용할 수 없고 맨 앞에 사용해야 한다.
...
-- 23. 사원번호, 성, 이름, 급여를 출력하시오.
select employee_id, last_name, first_name, salary
from employees;         -- 원하는 열만 출력 : 프로모션

-- 24. 90인 부서의 사원들만 출력할 때 사원번호, 이름, 성, 급여를 출력하시오.
select employee_id, first_name, last_name, salary
from employees              -- 프로젝션
where department_id = 90;   -- 셀렉션

-- 25. 이름이 'Nancy'인 사원의 입사일, 직무, 급여, 커미션퍼센트를 출력하시오.
...
-- hire_date의 데이터 타입이 date나 timestamp로 되어 있는 경우
select * from employees
...
-- 31. 급여가 2500에서 3500사이인 사원들 중 사원번호, 이름, 급여, 직무, 입사일을 출력하시오.
select employee_id, first_name, salary, job_id, hire_date
from employees
where salary between 2500 and 3500;

-- 32. 성이 King부터 Smith 사이에 있는 사원들을 출력하시오.
select * from employees 
where last_name between 'King' and 'Smith';

-- 33. 입사일이 02/06/07부터 06/01/03 사이에 입사한 사원들을 출력하시오.
select * from employees 
...
-- like (검색엔진에서 많이 쓰인다.)
-- 38. 성에 s가 포함되어 있는 직원을 출력하시오.
select * from employees where last_name like '%s%';
-- select * from board where contents like '%홍길동%';
...
-- 41. 이름이 'B'로 시작하는 직원을 출력하시오.
select * from employees where first_name like 'B%';
select * from employees where first_name like 'b%'; -- 대소문자 구분 

-- 42. 이름이 'a'로 끝나는 직원을 출력하시오.
select * from employees where first_name like '%a';

-- 43. 02년도에 입사한 사원은?
select * from employees where hire_date like '02%';

-- 44. 02월에 입사한 사원은?    -- 답 : 52번
-- select * from employees where hire_date like '%02%';
-- 02/01/01 , 01/02/01, 01/01/02
...
-- 65. 부서가 100이면서 직무가 'SA_REP'인 사원ㅇ르 출력하시오.
select * from employees where department_id = 100 and job_id = 'SA_REP';

-- 66. 직무가 'AD_PRES', 'AD_VP', 'IT_PROG'인 사원을 출력하시오.
...
----- 직무가 IT_PROG이거나 직무가 ST_MAN이면서 급여가 6000이상인 사원을 출력
...
-- and가 or보다 우선순위가 높기 때문에 무조건 and 부터 먼저
...
--- 정렬 (내림차순, 오름차순)
select * from employees;        -- primary key 기준으로 정렬

select salary, employee_id, first_name, hire_date
from employees;

-- 71. 급여를 기준으로 오름차순 정렬되어 급여, 사원번호, 이름, 입사일을 출력하시오.
select salary, employee_id, first_name, hire_date
from employees
order by salary;
...
-- 73. 50번 부서의 사원들을 출력할 때 입사일이 제일 늦은 사람부터 출력하시오. 내림차순
select * from employees
where department_id = 50
order by hire_date DESC;

-- 74. 50번 부서의 사원들을 출력할 때 급여가 제일 적은 사람부터 출력하시오.
select * from employees
where department_id = 50
order by salary;

-- 75. 급여가 6000이상 10000이하인 사원들을 입사일이 빠른 기준으로 출력하시오.
select * from employees
where salary BETWEEN 6000 and 10000
order by hire_date;

-- 76. 사원번호, 이름, 입사일, 급여, 부서번호를 출력할 때 급여를 제일 많이 받는 사원부터 출력하시오. 급여에 sal로 별칭을 사용한다.
...
select employee_id, first_name, hire_date, salary as sal, department_id
from employees
order by sal desc;      -- 정렬에서만 별칭을 사용할 수가 있다.

--          1           2           3           4               5
select employee_id, first_name, hire_date, salary as sal, department_id
from employees
order by 4 desc;        -- 정렬에서만 컬럼 순서를 사용할 수 있다.
...
-- 79. 부서를 오름차순으로 정렬하고 각부서에서 직무가 내림차순으로 정렬된 상태에서 직무에 따른 급여를 제일 많이 받는 사람부터 출력되게 하세요.
select * from employees
...
-- 급여를 받는 사람들의 입사일을 오름차순으로 정렬하시오.
select * from employees
order by department_id, job_id desc, salary, hire_date;

-- 81. 성이 Austin부터 Sciarra 사이에 있는 사원들을 부서별 급여를 모두 내림차순으로 정렬하세요.
select * from employees
where last_name between 'Austin' and 'Sciarra'
order by department_id desc, salary desc;

-- 82. 성이 Austin부터 Sciarra 사이에 있는 사원들을 직무별 급여를 모두 내림차순으로 정렬하세요.
select * from employees
where last_name between 'Austin' and 'Sciarra'
