...
public class LoginDAO extends DataBaseInfo{
	public AuthInfo loginCk(String id, String pw) {
		...
		con = getConnection();
		String sql = " select emp_id, emp_pw, 'emp' grade from employees where emp_id = ?"
				+ " union"
				+ " select mem_id, mem_pw, 'mem' from member where mem_id = ?";
		...
		try {
			...
			} // if가 아니면 AuthInfo 객체는 null
			
			/*
			if(rs.next()) { // 해당 id가 존재
				if(pw.equals(rs.getString("emp_pw"))) { // 헤딩네임
					i = 1; // pw 일치 - login 성공
				}else {
					i = 0; // pw 불일치
				}
