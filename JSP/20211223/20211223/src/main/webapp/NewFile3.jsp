...
<%
	...
	String str1 = new String("jsp웹 애플리케이션");
...
%>
	...
	str3의 4번째 index에 있는 문자 : <%= str3.charAt(4) %><br />
	str3에서 "ing"가 있는 index는 : <%= str3.indexOf("ing") %> <br />
	str3에서 index가 4이후에 있는 "st"는 몇번째 index인가? <%=str3.indexOf("st",4) %><br />
	...
	str3을 공백문자를 구분자로 문자열을 잘라 변수에 저장하고 자른 문자열을 저장한 변수를 이용하여 값을 출력 : <br />
<%
	String[] s = str3.split(" ");
...
<%
	...
	// matrix 에 있는 값을 출력	
	out.print("matrix.length의 배열의 크기 : " + matrix.length + "<br />");
	out.print("matrix[0].length의 배열의 크기 : " + matrix[0].length + "<br />");
	out.print("matrix[1].length의 배열의 크기 : " + matrix[1].length + "<br />");
...
<%
	...
	for(int i = 0; i < strMatrix1.length; i++) {
		for(int j = 0; j < strMatrix1[i].length; j++) { %>
			strMatrix1[<%=i %>][<%= j %>] = <%= strMatrix1[i][j] %><br />
<%		}
	}
%>
