...
//		}
    ...
    public List<StoreObjectReviewDTO> nameSearch(Map<String, Object> param) {
    	return sqlSession.selectList("Store.nameSearch", param);
    }
    
    public List<ObjectOrderPaymentStoreDeliveryDTO> orderList(Map<String, Object> param) {
    	param.put("startIndex", param.get("startIndex"));	
		param.put("endIndex", param.get("endIndex"));
    	return sqlSession.selectList("Store.orderList", param);
    }
    
    public int orderCheck(Map<String, Object> param) {
    	// 트렌젝션 구현
    	DefaultTransactionDefinition def = new DefaultTransactionDefinition();
    	def.setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRED);
    	TransactionStatus status = transactionManager_sample.getTransaction(def);
    	...
    	int result = 0;
    	try {
    		result = sqlSession.update("Store.orderCheck", param);
    		transactionManager_sample.commit(status);
            logger.info("========== orderCheck 완료 : {}", result);
    	}catch(Exception e) {
    		logger.error("[ERROR] orderCheck() Fail : e : {}", e.getMessage());
    		e.printStackTrace();
        	transactionManager_sample.rollback(status);    	
    	}
    	return result;
    ...
    public List<ObjectOrderPaymentStoreDeliveryDTO> menuList(Map<String, Object> param) {
    	return sqlSession.selectList("Store.menuList", param);
    }
